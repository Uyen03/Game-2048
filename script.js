document.addEventListener('DOMContentLoaded', () => {
    ...
    const size = 4;
    let board = [];
    let currentScore = 0; //Điểm hiện tại của người chơi.
    const currentScoreElem = document.getElementById('current-score');

    // Get the high score from local storage or set it to 0 if not found
    //localStorage: Một API của trình duyệt cho phép lưu trữ dữ liệu dưới dạng key-value, dữ liệu này sẽ tồn tại ngay cả khi người dùng tải lại trang hay đóng trình duyệt. 
    //Trong đoạn code này, highScore được lưu trữ dưới key là '2048-highScore'.
            ...
            for (let j = 0; j < size; j++) {
                ...
                if (currentValue !== 0) {
                    ...
                    if (currentValue !== parseInt(prevValue) && !cell.classList.contains('new-tile')) {
                        cell.classList.add('merged-tile'); //merged-tile css gộp ô 
                    }
                } else {
                    cell.textContent = '';
    ...
    function placeRandom() {
        ...
        //giá trị của ô mới được thêm vào là 2 hay 4. Có 90% khả năng giá trị sẽ là 2 và 10% khả năng là 4.
